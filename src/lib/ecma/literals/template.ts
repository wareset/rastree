...
import { replace, slice, last, endsWith } from 'wareset-utilites'
import { REG_ESCAPE_SEQUENCES, ESCAPE_SEQUENCES } from '../format-controls'

// prettier-ignore
export const createTemplateValue = (s: string): string =>
  replace(replace(slice(s, 1, last(s) === '`' ? -1 : endsWith(s, '${') ? -2 : undefined),
    /\\([\\`])|\\(\$)(?=\{)|(?<=\$)\\(\{)/g, '$1$2$3'),
