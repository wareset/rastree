...
export const typingNumeric: ITokenizerTypingFn = ({ options }: ITokenizer) => (
  token: IToken
): any => {
  const value = token.raw
  if (!token.type! && isNumericLiteral(value)) {
    token.type = TYPE_NUMERIC
    if (options.values) token.value = createNumericLiteralValue(value)
